...
def main():
    ...
    btn_1 = Button(APP_WINDOW, text="Запустить Edge", command=GIS_init, width=25)
    ...
    lbl_about = Label(APP_WINDOW, text="Бот для ответа на запросы по льготам ГИС ЖКХ :: версия 2.12 Edge")
...
def GIS_init(message=None, refresh_only=False):
    ...
    edge_options.add_argument("--wm-window-animations-disabled")
    
    edge_options.add_experimental_option('excludeSwitches', ['enable-logging'])
    GIS = webdriver.Edge(options=edge_options)
...
def GIS_go():
    ...
    if GIS.current_url == "edge://new/" or "no-privileges" in GIS.current_url:
...
## основной цикл обработки
    ...
    while True:
        ...
        if cc is None:
            ...
            GIS.get("edge://new/") # закрыть окно ГИС чтобы не мешать другим ботам
            break
...
##                row = row - 1
                GIS_init(message="Перезапуск Edge после фатальной ошибки", refresh_only=True)
                continue
